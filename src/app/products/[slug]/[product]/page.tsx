        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl p-8 flex items-center justify-center min-h-[300px]">
              {imgUrl ? <img src={imgUrl} alt={product.name} className="max-h-72 object-contain" /> : <Package size={80} className="text-white/50" />}
            </div>
            <div>
          ...
          <div className="container-wide">
            <div className="flex gap-1 mb-6 overflow-x-auto pb-2">
              {[{id:'overview',label:'Overview',icon:Info},{id:'application',label:'Application',icon:Paintbrush},{id:'properties',label:'Properties',icon:Shield},{id:'safety',label:'Safety & Packing',icon:AlertTriangle}].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium whitespace-nowrap transition ${activeTab === tab.id ? 'bg-[var(--color-navy)] text-white shadow' : 'bg-white text-gray-600 hover:bg-brand-50'}`}>
            ...
            {activeTab === 'overview' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {tds.benefits && (
                  <div className="bg-white rounded-xl p-6 shadow-sm">
                    <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Info size={18} className="text-[var(--color-navy)]" /> Product Benefits</h3>
                    <ul className="space-y-2">{tds.benefits.map((b: string, i: number) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-gray-700"><ChevronRight size={14} className="text-[var(--color-navy)] mt-0.5 shrink-0" /> {b}</li>
                    ))}</ul>
                  </div>
                )}
