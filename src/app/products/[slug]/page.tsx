      ...
      {categories.length > 0 && (
        <section className="bg-white border-b border-gray-100 py-4 sticky top-16 md:top-20 z-30 backdrop-blur-sm bg-white/95">
          <div className="container-wide px-4 flex gap-2 overflow-x-auto pb-1">
            <button onClick={() => setActiveCategory('all')} className={`badge font-medium whitespace-nowrap ${activeCategory === 'all' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-blue-50'}`}>All Products</button>
            {categories.map((cat: any) => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`badge whitespace-nowrap ${activeCategory === cat.id ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-blue-50 hover:text-[var(--color-navy)]'} cursor-pointer transition`}>{cat.name}</button>
            ))}
          </div>
        </section>
      )}
                ...
                return (
                  <Link key={product.id} href={`/products/${slug}/${product.slug}`} className="card card-hover group">
                    ...
                    <div className="p-5">
                      ...
                      <h3 className="text-lg font-bold text-gray-800 mb-1 group-hover:text-[var(--color-navy)] transition">{product.name}</h3>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        ...
                        <span className="text-[var(--color-navy)] text-sm font-medium flex items-center gap-1">Details <ArrowRight size={14} /></span>
                      </div>
                    </div>
                  </Link>
                );
