...
export default function SpecificationGenerator() {
  ...
  const doPrint = (mode: "spec" | "check") => { printMode.current = mode; setTab(mode === "check" ? "check" : "spec"); setTimeout(() => window.print(), 60); };
      ...
      {gate && (
        <div className="print:hidden fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setGate(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-[17px] font-semibold text-[#0B2A5B] mb-1">Download the specification</h3>
            <p className="text-[12.5px] text-slate-600 mb-5">
              Tell us who it is for and our technical team can review it with you — or skip and download anyway.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-5">
              <a href={gName && gPhone ? `https://wa.me/${WHATSAPP}?text=${gateText}` : undefined}
                target="_blank" rel="noopener noreferrer"
                onClick={() => { if (gName && gPhone) { setGate(false); setTimeout(() => doPrint("spec"), 300); } }}
                className={`flex-1 text-center text-[13px] font-medium py-2.5 rounded-lg ${gName && gPhone ? "bg-[#1E5AA8] text-white hover:bg-[#164683]" : "bg-slate-200 text-slate-400 pointer-events-none"}`}>
                Send &amp; download
              </a>
              <button onClick={() => { setGate(false); setTimeout(() => doPrint("spec"), 200); }}
                className="flex-1 border border-slate-300 text-slate-700 text-[13px] py-2.5 rounded-lg hover:border-slate-400">
                Just download
              </button>
            </div>
            <p className="text-[11px] text-slate-400 mt-3">
              We ask, we do not insist. The specification downloads either way.
            </p>
          </div>
        </div>
      )}
